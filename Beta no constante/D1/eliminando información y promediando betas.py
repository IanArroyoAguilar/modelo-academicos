...
def graf(last,ext):
    D_usable = np.array([1912,1846,1844,1996,2099,2154,2110,2122,1917][:-last])
    B_usable = np.array([226, 449, 713, 927, 1085, 1279, 1437, 1629, 1852][:-last])
    ...
    for k in range(N-1-last,N-1):
            ...
            beta_last = sum(b[k-ext:k])/len(b[k-ext:k])
            first, second = A(k,k,beta_last, D_fic),A(k,k+1,beta_last, D_fic)
            Dnew.append(second)
            print("modelo: ",np.round(second,3)," ~ Valor Real: ",D[k+1])
    ...
    for k in range(N-1-last,N-1):
            ...
            beta_last = sum(b[k-ext:k])/len(b[k-ext:k])
            first, second = Baj(k,k,beta_last,D_usable,B_fic),Baj(k,k+1,beta_last,D_usable,B_fic)
            Bnew.append(second)
            print("modelo: ",np.round(second,3)," ~ Valor Real: ",B[k+1])
...
# El primer parámetro indica la cantidad de datos que se eliminan
# de la información disponible de académicos y bajas.
# Es decir, la cantidad de años que se intentan predecir.

# El segundo parámetro indica la cantidad de betas que se promedian
# para hacer la predicción. Al hacer la predicción se incluye
# en el promedio ese valor para calcular la siguiente beta
# y así sucesivamente.
