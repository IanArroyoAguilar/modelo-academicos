...
F = [227,301,325,407,490,546,517,509,571,571+20,571-30,571+40]
...
def graficar_doc():
    ...
    ax2.scatter(range(9),B,label = "Datos")
    ...
    ax2.plot(oldt,B_curva_old,label = "Ajuste",color="blue")
    
    ax1.plot(newt,D_curva_new,label = "Predicción",color="red",linestyle="dotted",linewidth=4)
    ax2.plot(newt,B_curva_new,label = "Predicción",color="red",linestyle="dotted",linewidth=4)
    
    
    ax1.set_title(label="Docentes")
    ax2.set_title(label="Bajas")  
    ax3.set_title(label="Entrada de personal")
    ax1.set_ylim(min(D)-100,max(D)+50)
    ax2.set_ylim(0,max(B)+200)    
    ax1.set_xlim(-1,12)
    ax2.set_xlim(-1,12)
    ...
    ax2.set_xticks(range(-1,12,2),labels=range(2013,2026,2))
